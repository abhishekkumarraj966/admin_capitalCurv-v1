...
import { getPlanById, updatePlan } from '@/services/purchaseApi';
import {
    Trash2,
    Info,
    Coins,
...
function EditPlanForm({ id }: { id: string }) {
    ...
    useEffect(() => {
        const fetchPlan = async () => {
            try {
                ...
                const plan = (res as any).result || (res as any).data || res;
                setForm({
                    name: plan.name || '',
                    accountType: plan.accountType || 'Equity',
                    price: plan.price || 0,
                    ...
                    overallDrawdown: plan.overallDrawdown || '',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            alert('Failed to update plan. Please check if all fields are correct.');
        } finally {
            setSaving(false);
        }
    };
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center p-20 space-y-4">
            ...
            <p className="text-gray-500 font-bold">Loading Plan Data...</p>
        </div>
    );
        ...
        <div className="max-w-4xl mx-auto space-y-8 pb-10">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-black text-gray-900 dark:text-white leading-none mb-1">Edit Plan Settings</h1>
                        <p className="text-gray-500 text-[10px] font-bold uppercase tracking-widest">Modifying: <span className="text-[#0F8235]">{form.name}</span></p>
                    </div>
                </div>



                <div className="flex items-center gap-3 bg-white dark:bg-[#021F17] p-2.5 px-4 rounded-2xl border border-gray-100 dark:border-[#021F17] shadow-sm self-start md:self-auto">
                    <input
                        type="checkbox"
                    ...
                    />
                    <label className="text-xs font-black text-gray-600 dark:text-gray-400 uppercase tracking-wider">Plan Active</label>
                </div>
            </div>
                ...
                <div className="bg-white dark:bg-[#021F17] rounded-[2.5rem] p-8 md:p-12 shadow-2xl shadow-emerald-500/5 border border-gray-100 dark:border-[#021F17] space-y-12">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        className="w-full px-5 py-3.5 bg-gray-50 dark:bg-gray-700/30 border border-gray-200 dark:border-gray-700 rounded-2xl outline-none focus:bg-white font-bold"
                                        required
                                    />
                    ...
                    <div className="space-y-6">
                        <div className="flex justify-between items-center">
                            ...
                            <button type="button" onClick={addFeature} className="text-xs font-black text-[#4A3AFF] hover:bg-indigo-50 px-3 py-1.5 rounded-lg transition-all">+ Add Perk</button>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {form.features.map((feature, index) => (
                                <div key={index} className="flex items-center gap-2 p-1.5 bg-gray-50/50 dark:bg-gray-700/20 rounded-xl border border-gray-100 dark:border-gray-700 group">
                    ...
                    {/* Footer Action */}
                    <div className="pt-6 flex flex-col md:flex-row items-center justify-between gap-6 border-t border-gray-50 dark:border-gray-700">
                        <div className="flex items-center gap-4">
                            <div className="p-3 bg-emerald-50 dark:bg-emerald-900/30 rounded-2xl">
                                <Coins className="text-emerald-600" size={24} />
                            </div>
                            <div>
                                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1">Current Account Price</p>
                        ...
                        >
                            ...
                            <span>{saving ? 'Updating...' : 'Save Changes Now'}</span>
                        </button>
                    </div>
                </div>
            </form>
...
export default function EditPlanPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
        <Suspense fallback={<div className="p-10 text-center font-bold text-gray-500 border-2 border-dashed rounded-3xl">Fetching plan details...</div>}>
            <EditPlanForm id={id} />
        </Suspense>
    );
}
